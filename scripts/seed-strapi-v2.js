 ...
 *
 * 用法：
 *   node scripts/seed-strapi-v2.js                    # 全量 seed
 *   node scripts/seed-strapi-v2.js --only pages       # 只 seed pages
 *   node scripts/seed-strapi-v2.js --only solutions,cases  # 只 seed solutions 和 cases
 */
...
async function ensureLocalizedDocument(strapi, uid, { slug, localesData, label }) {
  ...
  // 查询所有语言版本的现有文档
  // 重要：需要为每个 locale 分别查询，因为 Strapi v5 Documents API 默认只返回一个 locale
  ...
  for (const locale of localesInOrder) {
    ...
    // 确保 payload 中包含 slug（Strapi v5 Documents API 要求每个 locale 都有 slug）
