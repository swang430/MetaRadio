...
Strapi 上传后会把文件名 slugify 并加 hash（如 `ds-v7-radar-hero.webp` → OSS 里 `metaradio/ds_v7_radar_hero_a1b2c3.webp`）。**上传前**起好名，OSS 里就清晰可查、不撞名：

- **datasheet hero 图**：`ds-<产品slug>-hero.<webp|jpg>`，如 `ds-v7-quantum-radar-hero.webp`（`<产品slug>` 与该 datasheet `.md` 的 `slug` 一致）。
- **规则**：全小写、连字符 kebab-case、**纯 ASCII**（无空格 / 中文，避免 OSS key 与 URL 乱码）、含「产品 slug + 用途」。
- **格式 / 尺寸**：首选 `.webp`（次选 `.jpg`）；hero 图 ≥ 1200px 宽、横向（约 3:2 或 16:9）、体积 < 500KB。
- 其余图（首页 / 海报）沿用既有命名：`hero-xxx`、`poster-xxx`、`dual-infrastructure` 等。

> 模板：新产品照 `seed-data/datasheets/_TEMPLATE.md` 复制填写（见上「新产品上线流程」第 3 步；`_` 前缀文件 import 时自动跳过）。
